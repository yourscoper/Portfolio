export default async function handler(req, res) {
  ...
  req.on('end', () => {
    try {
      const { json, filename } = JSON.parse(body || '{}');
...
<script>
  ...
  document.getElementById('decodeBtn').addEventListener('click', async () => {
    ...
    try {
      ...
      btn.disabled = false;
      btn.innerHTML = '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg> Decode &amp; Download';
    }
  });
  ...
  (function drawS() {
    ...
    for (const s of stars) {
      const a = Math.max(0.05, Math.min(0.85, s.base + Math.sin(t * 0.7 + s.phase) * 0.3));
      sx.fillStyle = \`rgba(240,248,255,\${a})\`;
  ...
  resizeTC(); window.addEventListener('resize', resizeTC);
  const pts = [];
  document.addEventListener('mousemove', e => pts.push({x:e.clientX, y:e.clientY, t:Date.now()}));
  (function drawT() {
    tx.clearRect(0,0,tc.width,tc.height);
    const now = Date.now();
    for (let i = pts.length-1; i >= 0; i--) {
      if (now - pts[i].t > 800) { pts.splice(0, i+1); break; }
    }
    for (let i = 1; i < pts.length; i++) {
      const age = (now - pts[i].t) / 800;
      tx.beginPath();
