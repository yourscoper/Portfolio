...
<head>
  ...
  <style>
    *{margin:0;padding:0;box-sizing:border-box}
    ...
    h1{font-size:clamp(3.5rem,12vw,8rem);color:#fff;text-shadow:0 0 40px rgba(255,255,255,.45),0 0 80px rgba(180,180,255,.25);margin-bottom:.4rem;letter-spacing:-2px;text-align:center}
    .version{font-size:1.6rem;color:#ddd;opacity:.9;margin-bottom:2.5rem;text-shadow:0 0 20px rgba(255,255,255,.3)}
    .editors{width:100%;max-width:1400px;display:grid;grid-template-columns:1fr 1fr;gap:1.5rem;margin-bottom:2rem}
    .editor-box{display:flex;flex-direction:column;height:520px;border:1px solid #222;border-radius:8px;overflow:hidden;background:#111;box-shadow:0 10px 40px rgba(0,0,0,.7)}
    .editor-label{padding:0.8rem 1.2rem;background:#1a1a1a;border-bottom:1px solid #222;font-size:1.3rem;color:#aaa}
    textarea,pre code{flex:1;width:100%;padding:1.2rem;font-size:1.05rem;line-height:1.55;font-family:Consolas,Monaco,'Andale Mono','Ubuntu Mono',monospace;background:#0f0f0f;color:#e0e0e0;border:none;outline:none;resize:none}
...
reset(){this.x=Math.random()*canvas.width;this.y=canvas.height+30;this.size=Math.random()*2.8+.8;this.speed=Math.random()*.7+.2;this.opacity=Math.random()*.5+.35;this.delay=Math.random()*3000;this.glow=Math.random()>.6?.9:.4}
update(t){if(t<this.delay)return;this.y-=this.speed;if(this.y<-20)this.reset()}
draw(){ctx.globalAlpha=this.opacity;ctx.shadowBlur=12*this.glow;ctx.shadowColor='#fff';ctx.fillStyle='#fff';ctx.beginPath();ctx.arc(this.x,this.y,this.size,0,Math.PI*2);ctx.fill()}}

for(let i=0;i<180;i++)particles.push(new Particle());

function animate(t=0){ctx.clearRect(0,0,canvas.width,canvas.height);particles.forEach(p=>{p.update(t);p.draw()});requestAnimationFrame(animate)}
animate();
...
document.getElementById('obfuscate').onclick=()=>{const code=input.value.trim();if(!code){highlightOutput("-- Nothing to obfuscate");return}
